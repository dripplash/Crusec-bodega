...
function stockValueFromItem(item) {
  ...
  return numberOrNull(firstValue(
    /*
     * Prioridad: stock disponible primero.
     * Esto evita tomar stock total cuando Relbase entrega ambos.
     */
    item.stock_disponible,
    item.stockDisponible,
    item.disponible,
    item.disponible_para_venta,
    item.disponibleParaVenta,
    ...
    /*
     * Luego stock/saldo general del registro de bodega.
     */
    item.stock,
    item.stock_actual,
    item.stockActual,
...
function warehouseText(item) {
  ...
  return cleanText(firstValue(
    /*
     * Nombres directos de bodega/almacén.
     */
    item.bodega_nombre,
    item.bodegaNombre,
    item.nombre_bodega,
    ...
    /*
     * Ubicación o referencias anidadas.
     */
    item.ubicacion,
    item.location,
    ...
    nestedName(item.sucursal),
    nestedName(item.location),
    nestedName(item.branch)
  )).toLowerCase();
}
...
function isMainWarehouseItem(item) {
  ...
  const configured = cleanText(process.env.RELBASE_MAIN_WAREHOUSE_NAME || '').toLowerCase();

  if (configured && text.includes(configured)) return true;
...
function firstStockFromArrays(product) {
  return (
    ...
    stockFromMainWarehouseArray(product.stock) ??
    stockFromMainWarehouseArray(product.stock_actual) ??
    stockFromMainWarehouseArray(product.stockActual) ??
    stockFromMainWarehouseArray(product.bodegas) ??
    stockFromMainWarehouseArray(product.warehouses) ??
    stockFromMainWarehouseArray(product.almacenes) ??
    stockFromMainWarehouseArray(product.sucursales) ??
    stockFromMainWarehouseArray(product.stock_bodegas) ??
    stockFromMainWarehouseArray(product.stockBodegas) ??
    stockFromMainWarehouseArray(product.stocks_bodegas) ??
    stockFromMainWarehouseArray(product.stocksBodegas) ??
    stockFromMainWarehouseArray(product.stock_warehouses) ??
    stockFromMainWarehouseArray(product.stockWarehouses) ??
    stockFromMainWarehouseArray(product.warehouse_stock) ??
    stockFromMainWarehouseArray(product.warehouseStock) ??
    stockFromMainWarehouseArray(product.stock_by_warehouse) ??
    stockFromMainWarehouseArray(product.stockByWarehouse) ??
    null
  );
}
...
function detectStock(product) {
  /*
   * Primero buscamos stock de Bodega principal.
   * La pantalla "Stock actual" de Relbase suele mostrar esa bodega, no el total.
   */
  const mainWarehouseStock = firstStockFromArrays(product);

  if (mainWarehouseStock !== null) return mainWarehouseStock;

  /*
   * Después usamos stock disponible directo del producto.
   */
  const availableDirect = numberOrNull(firstValue(
    product.stock_disponible,
    product.stockDisponible,
    product.disponible,
    product.disponible_para_venta,
    product.disponibleParaVenta,
  ...
  /*
   * Último recurso directo: stock general.
   * Puede ser stock total, por eso va después.
   */
  ...
  /*
   * Último recurso de respaldo: suma de arrays, solo si no encontramos nada mejor.
   */
  return (
    ...
    sumStockArray(product.almacenes) ??
    sumStockArray(product.sucursales) ??
    sumStockArray(product.stock_bodegas) ??
    sumStockArray(product.stockBodegas) ??
    sumStockArray(product.stocks_bodegas) ??
    sumStockArray(product.stocksBodegas) ??
    sumStockArray(product.stock_warehouses) ??
    sumStockArray(product.stockWarehouses) ??
    sumStockArray(product.warehouse_stock) ??
    sumStockArray(product.warehouseStock) ??
    sumStockArray(product.stock_by_warehouse) ??
    sumStockArray(product.stockByWarehouse) ??
    null
  );
}
...
function normalizeProduct(product, syncedAt = null) {
  ...
  return {
    ...
    stockUpdatedAt: syncedAt || product.stockUpdatedAt || product.updated_at || product.updatedAt || product.fecha_actualizacion || null,
  };
}
...
async function listProducts(options = {}) {
  ...
  const normalized = all
    .map((product) => normalizeProduct(product, syncedAt))
...
async function findProductBySku(skuInput) {
  ...
  for (const url of urls) {
    ...
    if (match) {
      return normalizeProduct(match, new Date().toISOString());
    }
    ...
    for (const candidate of singleProductCandidates(payload)) {
      const normalized = normalizeProduct(candidate, new Date().toISOString());
